...
#### Класс WebLarekAPI
Содержит методы, предназначенные для взаимодействия с бэкэндом. Наследуется от класса `Api`, имплементирует интерфейс `IWebLarekAPI`. Содержит доступное только для чтения поле `cdn`. В конструкторе принимает поля `cdn`, базового URL `baseURL` и настроек `options`.
