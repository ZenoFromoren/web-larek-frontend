...
#### Класс View<T>
Наследуется от класса `Component<T>`. Является дженериком, принимает тот же тип `T`, что и `Component`. Отвечает за отображение элементов, генерирующих события. От данного класса наследуются классы  `Modal` и `Page`.
...
Также Содержит методы `set` для установки значений полей адреса и спопоба оплаты.
...
#### Класс Modal
Описывает элемент модального окна оформления заказа. Содержит поле, описывающее содержащийся внутри модального окна контент и поле кнопки оформления заказа. Также содержит методы `open` и `close`, отвечающие за отображение и скрытие модального окна соответственно. Наследуется от класса `View<IModalData>`.

#### Класс Form<T>
Описывает элемент формы заказа. Является дженериком, принимает тип `T`, ключи которого используются как аргументы в методах класса. Наследуется от класса `View<IFormState>` с имплементированным интерфейсом. 
Содержит следующие защищённые поля:
...
- элемент, отображающий ошибки валидации формы
 ...
 - `render` - переопределяет родительский метод `render`, добавляя поле объекта состояний валидации формы, ошибок, данных, введённых в поля ввода
 - методы `set` для установки значений параметров
...
#### Класс WebLarekAPI
Предназначен для взаимодействия с бэкэндом. Наследуется от класса `Api`, имплементирует интерфейс `IWebLarekAPI`. Содержит доступное только для чтения поле `cdn`. В конструкторе принимает поля `cdn`, базового URL `baseURL` и настроек `options`.
Содержит следующие методы, предназначенные для взаимодействия с бэкэндом:
- `getProductsList` - получение списка продуктов
- `getProductItem` - получение отдельного продукта
- `createOrderProducts` - оформление заказа
...
#### Модель данных
#### Класс AppData
Содержит в себе бизнес-логику хранения и изменение данных приложения. 
